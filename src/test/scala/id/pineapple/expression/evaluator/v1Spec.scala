...
import id.pineapple.expression.evaluator.v1.{Parser, PrintEvaluator, SimpleEvaluator}
...
class v1Spec extends FlatSpec {
	...
	"A Parser and PrintEvaluator" should "correctly process expression 2 + 2 * 2" in {
		...
		assert(new Parser {}.parseExpression(SourceText("2 + 2 * 2", "test.txt").headToken)._2.stop.char == 10)
		assert(new Parser {}.parseExpression(SourceText("2 + 2 * 2 ", "test.txt").headToken)._2.stop.char == 10)
	}
